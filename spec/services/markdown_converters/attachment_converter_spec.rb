...
RSpec.describe MarkdownConverters::AttachmentConverter do
  ...
  context '#add_file_properties' do
    it 'can find images from a body of markdown' do
      expect(MarkdownConverters::AttachmentConverter.add_file_properties(original_article.body)).to include('(PDF, 243KB)')
    end
  end
  ...
  context '#extract_names_and_links' do
    context 'no names and links' do
      it 'can find images from a body of markdown' do
        expect(MarkdownConverters::AttachmentConverter.send(:extract_names_and_links, "Some Text")).to eq([])
      end
    end

    context 'with names and links' do
      it 'can find images from a body of markdown' do
        expect(MarkdownConverters::AttachmentConverter.send(:extract_names_and_links, original_article.body)).to eq([["Parliamentary Evolution", "https://www.parliament.uk/documents/parliamentary-archives/evolution.pdf"], ["Wikipedia", "https://en.wikipedia.org/wiki/Portable_Document_Format"]])
      end
    end
  end
  ...
  context '#valid_file_format' do
    context 'invalid file format' do
      it 'returns false' do
        expect(MarkdownConverters::AttachmentConverter.send(:valid_file_format?, '.html')).to eq false
      end
    end

    context 'valid file format' do
      it 'returns true' do
        expect(MarkdownConverters::AttachmentConverter.send(:valid_file_format?, '.pdf')).to eq true
      end
    end
  end
  ...
  before do
    MarkdownConverters::AttachmentConverter.send(:open_and_save_file, file, file_destination)
  end
  ...
  context '#find_file_size', specific_specs: true do
    it 'returns file size in kilobytes' do
      expect(MarkdownConverters::AttachmentConverter.send(:find_file_size, file_destination)).to eq 243
    end
  end

  context '#find_file_type', specific_specs: true do
    it 'returns file type' do
      expect(MarkdownConverters::AttachmentConverter.send(:find_file_type, '.pdf')).to eq 'PDF'
    end
  end

  context '#delete_file' do
    it 'deletes a file from a given destination' do
      MarkdownConverters::AttachmentConverter.send(:delete_file, file_destination)
  ...
  context '#set_new_attachment_name' do
    it 'sets new attachment name, so that it includes file type and size' do
      expect(MarkdownConverters::AttachmentConverter.send(:set_new_attachment_name, original_article.body, 'Parliamentary Evolution', 'PDF', 243)).to include('[Parliamentary Evolution (PDF, 243KB)]')
    end
  end
