  ...
  class AttachmentConverter
    class << self
      ...
      def add_file_properties(markdown)
        ...
        file_desintation          = "#{Rails.root}/tmp/#{index}"

        extracted_names_and_links.each_with_index do |extracted_name_and_link, index|
          return unless valid_file_format?(file_extension)

          name = extracted_name_and_link[0]
          link = extracted_name_and_link[1]

          open_and_save_file(link, file_desintation)

          file_size = find_file_size(file_desintation)
          file_type = find_file_type(file_extension)

          delete_file(file_desintation)

          set_new_attachment_name(name, file_type, file_size)
        end
    ...
    def set_new_attachment_name(old_name, file_type, file_size)
